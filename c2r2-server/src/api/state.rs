...
impl ApiState {
    ...
    /// Add a new agent
    pub async fn add_agent(&self, id: ClientId, info: AgentInfo, tx: mpsc::UnboundedSender<String>) {
        {
            let mut agents = self.agents.write().await;
            agents.insert(id, AgentState { info: info.clone(), tx: tx.clone() });
        }
        {
    ...
    pub async fn update_agent_info(&self, id: ClientId, updater: impl FnOnce(&mut AgentInfo)) {
        ...
        if let Some(agent) = agents.get_mut(&id) {
            ...
            let _ = self.event_tx.send(ServerEvent::AgentUpdated(agent.info.clone()));
        }
    }
